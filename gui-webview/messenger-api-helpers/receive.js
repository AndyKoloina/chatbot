...
const handleStore = (senderId,store) => {
  sendApi.sendStoreMessage(senderId,store);
}

const handleRestaurants = (senderId,restaurant) => {
  sendApi.sendRestaurantMessage(senderId,restaurant);
}
...
const handleReceivePostback = (event) => {
  ...
  case 'GET_STARTED':
    ...
    break;
  case 'LET_GO':
      sendApi.sendChooseMenutMessage(senderId);
      break;
  case 'HOT_OFFER':
    sendApi.sendDealsCarousel(senderId);
    break;
  case 'EVENT_NEWS':
    sendApi.sendEventNewsCarousel(senderId);
    break;
  case 'EVENT_DEALS_NEWS':
    sendApi.sendEventNewsDealsMessage(senderId);
    break;
  case 'STORES_RESTAURANTS':
    sendApi.sendStoresRestaurantsMessage(senderId);
    break;
  case 'DIRECT_US':
    sendApi.sendDirectUsMessage(senderId);
    break;
    
  case 'STORES':
      sendApi.sendStoresMessage(senderId);
      break;
  case 'RESTAURANTS':
      sendApi.sendRestaurantsMessage(senderId);
      break;
  case 'MAIN_MENU':
      sendApi.sendWelcomeMessage(senderId);
      break;
...
const handleReceiveMessage = (event) => {
  ...
  if (message.text) { sendApi.sendAutoResponseMessage(senderId);}
};
